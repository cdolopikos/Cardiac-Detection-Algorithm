...
#     # print("ecg_detect_peaks", ecg_detect_peaks)
#     # ecg_relmax_peaks1 = scipy.signal.argrelmax(np.abs(ecg_data), order=200)
#     # print("ecg_relmax_peaks1", ecg_relmax_peaks1)
#     ecg_relmax_peaks = scipy.signal.argrelmax(np.abs(ecg_data), order=200)[0]
#     # print("ecg_relmax_peaks", ecg_relmax_peaks)
#
#     if len(ecg_relmax_peaks) > 1:
#
#         final_peaks = []
#         for peak in ecg_relmax_peaks:
...
#
#         laser_peaks = mmt.find_peaks.find_peaks_cwt_refined(-laser_data, np.array([20, 50, 100, 150, 200, 300, 500]),
#                                                             decimate=True, decimate_factor=1)
#
#         # print("laser_peaks", laser_peaks)
#         outer_delay = np.subtract.outer(ecg_peaks_sample, laser_peaks)
#         # print("outer_delay", outer_delay)
#         # outer_delay[outer_delay >= 0] = -100000
#         # print("outer_delay", outer_delay)
#
#
#         outer_delay_max = np.max(outer_delay, axis=1)
#         # print("outer_delay_max", outer_delay_max)
#
...
# #     calc_magic_laser(ecg_data=ecg_out, laser_data=per_out, no_log=False)
