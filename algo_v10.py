    ...
    while True:
        ...
        try:
            ...
            if np.mean(bp_out)<2:
                bp_out=bp_out*100
            # fig, ax = plt.subplots()
            # ax.plot(per_out,label="Filtered Perfusion")
            # # ax.plot(per_out,label="Filtered Perfusion")
            # ax.legend()
            # ax.set_xlabel(r'Time (ms)')
            # ax.set_ylabel(r'Signal Strength')
            # ax.set_title('Filtered Perfusion')
        ...
        for start_global_time, end_global_time in peak_pairs_to_process:
            ...
            if num_lasers==1:
                ...
                perfusion_amplitude_laser = np.log(perfusion_consensus_max*10)-np.log(perfusion_consensus_min)
                if np.isinf(perfusion_amplitude) or np.isinf(perfusion_amplitude_laser):
                ...
                update_dict = {
                    "BP Estimat": bp_inteerval,
                               ...
                               "Magic Laser": abs((perfusion_amplitude_laser)),
                ...
                perfusion_amplitude2_laser = np.log(perfusion_consensus_max2*10) - np.log(perfusion_consensus_min2)
